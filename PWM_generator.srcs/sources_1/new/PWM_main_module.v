...
//FREQUENCY AND DUTY-CYCLE INDEX INCREMENT -> FREQUENCY SELECTION -> DUTY-CYCLE CALCULATION -> ACTIVE VALUE STORAGE -> PWM SIGNAL GENERATION;

//REQUIREMENTS:
//  *10 period modes - 100 to 1000 clock cycles; Step - 100 clk cycles; PWM signal frequency 1MHz to 100kHz;
//  *11 duty-cycle modes - 0% to 100%; Step - 10%;
//  *Frequency and duty-cycle changes must be applied at the start of a new PWM period, without glitches;
//  *At 0% duty cycle, the PWM output must remain LOW;
//  *When the lowest frequency is reached, the next button press must return frequency to 1 MHz;
    ...
    output PWM_signal                   //PWM output signal;
        ...
        always@(posedge clk) begin //Block holds currently active PWM period and duty-cycle;
            if (rst) begin
                ...
                if (PWM_counter >= active_T_PWM-1) begin    //When counter reaches PWM period, it gets new frequency and duty-cycle values;
